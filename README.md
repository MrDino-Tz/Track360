...
Manufacturing equipment incident reporting over ordinary SMS and USSD.

A worker sends `M04 machine stopped working` to an Africa's Talking number. Africa's Talking posts that message to this backend. The backend stores the SMS, creates an incident, and the dashboard shows it within a few seconds. Workers can also type just a code (e.g. `COMP01`) for a follow-up conversation that prompts for the fault condition — with questions generated by Groq.

**Send an SMS about an equipment problem, and it becomes a trackable incident on the manufacturing dashboard.**

It supports two channels: **SMS** (one-way ingest plus two-way conversation) and **USSD**, plus a dashboard **AI assistant** (Groq) for shift admins.
...
Africa's Talking
  ...
  → HTTP POST /webhooks/africastalking/ussd
FastAPI
  → SQLite
  → REST API
  → Thibitisha dashboard
```
...
6. AI Assistant — chat with the Groq maintenance assistant
...
| POST | `/webhooks/africastalking/ussd` (alias `POST /ussd`) |
| POST | `/api/sms/sync` |
| POST | `/api/ai/chat` |
| GET | `/api/equipment` |
| POST | `/api/equipment` |
...
| POST | `/api/incidents/{id}/reward` |
| GET | `/api/incidents/{id}/rewards` |
